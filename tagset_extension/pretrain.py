...
gluster_path = "/glusterfs/dfs-gfs-dist/goldejon/ner4all/loss_function_experiments/fewnerd_fixed_targets/{loss_fn}/"
pretraining_extension = "pretraining/{semantic_level}-{num_labels}-labels-seed-{seed}"

def pretrain_fixed_targets(args):
    experiment_path = gluster_path.format(loss_fn=args.loss_fn) + pretraining_extension

    if not os.path.exists(gluster_path.format(loss_fn=args.loss_fn)):
        os.makedirs(gluster_path.format(loss_fn=args.loss_fn))

    file_handler_pretraining = logging.FileHandler(
        f'/glusterfs/dfs-gfs-dist/goldejon/ner4all/loss_function_experiments/fewnerd_fixed_targets/{args.loss_fn}/{task}.log'
    )
