...
def pretrain_fixed_targets(args):
    ...
    for config, indices in pretraining_indices.items():
        ...
        if args.dataset == "fewnerd":
            ...
            pretraining_extension = "-{transformer}-{entity_mentions}-dataset-with-{zelda_sampling}-and-{num_negatives}-negatives-excluded-{label_column}-seed-{seed}".format(
                transformer=transformer,
        ...
        if args.dataset == "fewnerd":
            ...
            pretraining_labels_qa = [
                semantic_label_name_map["few-nerd"][f"{label_column}_{args.label_semantic_level}"].get(x) for x in
                pretraining_labels]
            assert set(pretraining_dataset.features[label_column].feature.names) - set("O") == set(pretraining_labels_qa)
            ...
            if label_column == "fine_ner_tags":
                mask_labels = list(set([x for y in pretraining_labels for x in y.split("-")]))
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, choices=["fewnerd", "zelda"])
    ...
    parser.add_argument("--zelda_sampling", type=str, choices=["full_desc", "sampled_desc"], required=False)
